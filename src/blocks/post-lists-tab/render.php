...
>
	<?php if ( count( $attributes['categories'] ) > 0 ) : ?>
		<div class="tab mb-10 flex gap-2 p-4 pl-0">
			...
			<?php
			foreach ( $attributes['categories'] as $newsly_category ) :
				?>
				<button type="button" data-cat-id="<?php echo esc_attr( $newsly_category['value'] ); ?>" data-cat-slug="<?php echo esc_attr( $newsly_category['slug'] ); ?>" aria-selected="false" class="tablinks no-underline px-4 py-2 font-semibold text-sm transition-all rounded-md capitalize border shadow-sm">
					<?php echo esc_html( $newsly_category['label'] ); ?>
				</button>
			<?php endforeach; ?>
		</div>
		<?php else : ?>
	...
	<div id="post-list-tab-post-content" class="post-list-tab-post-content grid grid-cols-3 gap-4">
		<?php if ( count( $attributes['fetchedPosts'] ) > 0 ) : ?>
			<?php foreach ( $attributes['fetchedPosts'] as $newsly_post ) : ?>
				<div class="post-card shadow-md hover:shadow-lg rounded border-solid border-x border-y p-8">
					...
					<div class="mb-4 thumbnail card__img rounded inline-block">
						<?php if ( has_post_thumbnail( $newsly_post['id'] ) ) : ?>
							<?php
								echo get_the_post_thumbnail( $newsly_post['id'], 'large', array( 'class' => 'post-thumbnail rounded h-80 object-cover w-full' ) );
							?>
						<?php else : ?>
					...
					<a class="post-card__title inline-block font-poppins text-xl transition font-medium" href="<?php echo esc_url( get_the_permalink( $newsly_post['id'] ) ); ?>">
						<h2><?php echo esc_html( $newsly_post['title']['rendered'] ); ?></h2>
					</a>

					<?php if ( $newsly_post_list_tab_show_category ) : ?>
					<div class="inline-block mt-2">
						<!-- Loop through categories -->
						<?php if ( count( $newsly_post['categories'] ) > 0 ) : ?>
							<?php
							foreach ( $newsly_post['categories'] as $newsly_post_category_id ) :
								$newsly_post_category = get_category( $newsly_post_category_id );
								?>
								<a href="<?php echo esc_url( get_category_link( $newsly_post_category->term_id ) ); ?>" class="post-card__category inline-block text-xs capitalize p-1 mr-1 rounded-md transition-all">
									<?php echo esc_html( $newsly_post_category->name ); ?>
								</a>
							<?php endforeach; ?>
						<?php endif; ?>
					</div>
					...
					<div class="post-card__excerpt mt-2"><?php echo wp_kses_post( $newsly_post['excerpt']['rendered'] ); ?></div>
					<?php endif; ?>
				</div>
			<?php endforeach; ?>
		<?php else : ?>
